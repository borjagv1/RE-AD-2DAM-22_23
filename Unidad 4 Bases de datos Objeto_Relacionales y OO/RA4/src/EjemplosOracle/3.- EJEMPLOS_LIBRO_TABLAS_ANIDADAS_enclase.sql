...
--acceso a filas de la columna que es tabla anidada(debe ser una unica fila por ello se utiliza un id)
-- LAS CALLES DEL ID 1
...
BEGIN
 ...
 --EL ID EXISTE, COMPROBAR SI LA CALLE EXISTE:
  SELECT DISTINCT INTO IDT ID FROM EJEMPLO_TABLA_ANIDADA, TABLE(DIREC)
            WHERE ID= 1 
            AND UPPER(CALLE)=UPPER('C/Segovia 23-3A') 
...
END;
/
-- CONSULTA PARA OBTENER EL Nº DE DIRECCIONES QUE TIENE EN CADA CIUDAD EL IDENTIFICADOR 1
SELECT ID, APELLIDOS, CURSOR (
                      SELECT count(*) 
...
EJERCICIO 4.6
==========================================================
SELECT CIUDAD,COUNT(CALLE)  FROM THE 
    ( SELECT DIREC 
      FROM EJEMPLO_TABLA_ANIDADA where ID=1 )   
GROUP BY CIUDAD ORDER BY 1;
/
==========================================================
